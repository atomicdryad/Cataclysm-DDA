...
void game::load_npcs()
{
    for (int i = 0; i < cur_om->npcs.size(); i++)
    {
        ...
        dbg(D_INFO) << stringfmt("game::load_npcs[%d]:\
 lev %d:%d map (%d:%d) pos %d,%d : dist(%d,%d ,map) <= %d, act: %s",i,
...
z.size(), active_npc.size(), events.size());
  ...
  for (std::vector<Skill*>::iterator aSkill = Skill::skills.begin(); aSkill != Skill::skills.end(); ++aSkill) {
    int skid=(*aSkill)->id();
    skmenu.addentry(skid+1, true, -1, "@ %d: %s  ",  (int)u.skillLevel(*aSkill),(*aSkill)->ident().c_str());
    origskills[skid]=(int)u.skillLevel(*aSkill);
  }
  do {
    skmenu.query();
    int skid=-1; int skset=-1;
    int sksel=skmenu.selected-1;
    ...
    if ( skset != -1 && skid != -1 ) {
      ...
      skmenu.entries[skid+1].txt = stringfmt("@ %d: %s  ",(int)u.skillLevel(Skill::skills[skid]),Skill::skills[skid]->ident().c_str());
