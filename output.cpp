...
std::string string_input_popup(std::string title, int max_length, std::string input, std::string desc, std::string identifier ) {
  ...
  int pos = ret.size();//utf8_width(input.c_str());
  int lastpos=pos;
  ...
  do {
    ...
    if (ch == 27) {	// Escape
      ...
      if(identifier.size() > 0 && ret.size() > 0 ) {
        ...
        if( hist!=NULL ) {
//          std::string lhist=(*hist)[hist->size()-1];
